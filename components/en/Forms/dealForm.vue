  ...
  <v-container>
    ...
    <v-form ref="form" v-model="valid">
      ...
      <v-row justify="center" dense class="mt-3">
        <v-col cols="10" md="5">
          <v-text-field v-model="dealData.name" counter='70' :rules="[(v) => !!v || 'Deal Name is required', (v) => v.length <= 70 || 'Max 70 characters']" label="Deal Name" required @input="setName" outlined dense />
        </v-col>

        <v-col cols="10" md="5">
          <v-text-field v-model="dealData.arName" counter='70' :rules="[(v) => !!v || 'اسم العرض مطلوب', (v) => v.length <= 70 || 'Max 70 characters']" label="اسم العرض" required @input="setArname" outlined dense />
        </v-col>

        <v-col cols="10" md="5">
          <v-select v-model="dealData.merchant" label="Merchant" :rules="[(v) => !!v || 'Merchant is required']" required :items="merchants" item-value="_id" item-text="name" @change="setMerchant" outlined dense />
        </v-col>

        <v-col cols="10" md="5">
          <v-select v-model="dealData.category" label="Category" :rules="[(v) => !!v || 'Category is required']" required :items="flatCategories" item-value="_id" item-text="name" @change="setCategory" outlined dense />
        </v-col>

        <v-col cols="10" md="5">
          <v-select v-model="dealData.branches" :multiple="true" label="Branch" required :items="branchesToShow" item-value="_id" item-text="name" @changed="setBranches" outlined dense />
        </v-col>

        <v-col cols="10">
          <v-row v-if="dealData.images.length" justify="center" class="text-center" dense>
            <v-col cols="5">
              <v-btn color="red" dark small @click="removeImage">Remove Cover</v-btn>
              <v-btn color='success' dark small @click='addMoreImages'>Add More</v-btn>
            </v-col>
            <v-col cols="12"></v-col>            
          ...
          <v-row justify="center" class="flex-column" dense v-show="!dealData.images.length">
            <v-btn color="success" ref="upBtn" dark>
        ...
        <v-col cols="10">
          <v-text-field v-model="dealData.video" label="Video Link" @input="setVideo" outlined dense />
        </v-col>

        <v-col cols="10" class="my-5">
          <h6 class="text-center">Highlights</h6>
          <vue-editor v-model="dealData.highlightes" placeholder="Highlights" :rules="[(v) => !!v || 'Highlights is required']" required @input="setHighlight" outlined dense></vue-editor>
          <!-- <v-textarea v-model="dealData.highlightes" placeholder="Highlights" :rules="[(v) => !!v || 'Highlights is required']" required @input="setHighlight" outlined dense /> -->
        </v-col>

        <v-col cols="10" class="my-5">
          <h6 class="text-center">Description</h6>
          <vue-editor v-model="dealData.description" placeholder="Description" :rules="[(v) => !!v || 'description is required']" required @input="setDescription" outlined dense></vue-editor>
          <!-- <v-textarea v-model="dealData.description" placeholder="Description" :rules="[(v) => !!v || 'description is required']" required @input="setDescription" outlined dense /> -->
        </v-col>

        <v-col cols="10" class="my-5">
          <h6 class="text-center">Rules or Fine Print</h6>
          <vue-editor v-model="dealData.finePrint" class="textEditor" placeholder="Fine Print" :rules="[(v) => !!v || 'Fine Print is required']" required outlined dense></vue-editor>
          <!-- <v-textarea v-model="dealData.finePrint" placeholder="Fine Print" :rules="[(v) => !!v || 'Fine Print is required']" required outlined dense /> -->
        </v-col>

        <v-col cols="10" class="my-5">
          <h5 class="text-center">ابرز المعلومات</h5>
          <vue-editor v-model="dealData.arHighlightes" placeholder="ابرز المعلومات" :rules="[(v) => !!v || ' ابرز المعلومات مطلوبه']" required @input="setArhighlight" outlined dense></vue-editor>
          <!-- <v-textarea v-model="dealData.arHighlightes" placeholder="ابرز المعلومات" :rules="[(v) => !!v || ' ابرز المعلومات مطلوبه']" required @input="setArhighlight" outlined dense /> -->
        </v-col>

        <v-col cols="10" class="my-5">
          <h5 class="text-center">الوصف</h5>
          <vue-editor v-model="dealData.arDescription" placeholder="الوصف" :rules="[(v) => !!v || 'Description ar is required']" required @input="setArDescription" outlined dense></vue-editor>
          <!-- <v-textarea v-model="dealData.arDescription" placeholder="الوصف" :rules="[(v) => !!v || 'Description ar is required']" required @input="setArDescription" outlined dense /> -->
        </v-col>

        <v-col cols="10" class="my-5">
          <h5 class="text-center">الشروط</h5>
          <vue-editor v-model="dealData.arFinePrint" placeholder="الشروط" :rules="[(v) => !!v || 'Fine Print ar is required']" required outlined dense></vue-editor>
          <!-- <v-textarea v-model="dealData.arFinePrint" placeholder="الشروط" :rules="[(v) => !!v || 'Fine Print ar is required']" required outlined dense /> -->
        </v-col>
      ...
      <v-row justify="center">
        <v-col cols="10" md="10">
          <v-select v-model="dealData.dealType" label="Deal Type" :items="dealTypes" item-value="val" item-text="name" @change="setDealType" outlined dense />
        </v-col>
       <!-- 
        ...
        -->
        <v-col cols="10" md="5">
          <v-text-field v-model="dealData.dealStartDate" :rules="[(v) => !!v || 'Date is required']" label="Start Date" required type="datetime-local" @input="setDate" outlined dense />
        </v-col>
        <v-col cols="10" md="5">
          <v-text-field v-model="dealData.dealEndDate" :rules="[(v) => !!v || 'Date is required']" label="End Date" required type="datetime-local" @input="setEndDate" outlined dense />
        </v-col>
      </v-row>
      ...
      <v-row justify="center">
        <v-col cols="10" class="d-flex justify-content-end">
          <v-btn color="info" small min-width="150" @click="add_price_form()">+ Add Price</v-btn>
        </v-col>
        ...
        <v-col cols="10" v-for="(tier, index) in dealData.prices" :key="index">
          <v-row justify="center" class="text-center" dense>
            <v-col cols="12" class="d-flex justify-content-between mb-3">
              ...
              <v-btn color="red" class="ml-3" min-width="150" @click="delete_price_form(index)" dark small>Delete price</v-btn>
            </v-col>
            <v-col cols="12" md="6">
              <span>Original Price</span>
              <v-text-field v-model="tier.originalPrice" hint="Original Price" type="number" required :rules="[(v) => !!v || 'Original Price is required']" outlined dense />
            </v-col>
            <v-col cols="12" md="6">
              <span>Final Price</span>
              <v-text-field v-model="tier.finalPrice" hint="Final Price" type="number" :rules="[(v) => !!v || 'Final Price is required']" required outlined dense />
            </v-col>
            <v-col cols="12" md="6">
              <span>From Date</span>
              <v-text-field v-model="tier.pricesDate.dateDayFrom" hint="From Date"  type="datetime-local" outlined dense />
            </v-col>
            <v-col cols="12" md="6">
              <span>To Date</span>
              <v-text-field v-model="tier.pricesDate.dateDayTo" hint="To Date"  type="datetime-local" outlined dense />
            </v-col>
            ...
            <v-col cols="12"  v-for="(s,s_index) in tier.size" :key="'s'+s_index">
              <v-row justify="center" class="text-center" dense>
                ...
                <!-- <v-col cols="10" md="6" class="mt-5">
                  <span>Size</span>
                  <v-text-field v-model="tier.size[s_index]" hint="Size" outlined dense/>
                </v-col>
                <v-col cols="12"></v-col>
                <v-col cols="10" md="6">
                  <v-btn v-if="s_index == 0" @click="tier.size.push(''); tier.color.push('');" color="info" block>Add color</v-btn>
                  <v-btn v-else @click="tier.size.splice(s_index,s_index); tier.color.splice(s_index,s_index);" color="red" dark block>remove</v-btn>
                </v-col> -->
                <v-col cols="12" class="my-5"></v-col>
                <v-col cols="10" class="my-5">
                  <h6 class="text-center">الوصف</h6>
                  <vue-editor v-model="tier.arDescription" placeholder="الوصف" :rules="[(v) => !!v || 'الوصف مطلوب']" required outlined dense></vue-editor>
                  <!-- <v-textarea v-model="tier.arDescription" placeholder="الوصف" :rules="[(v) => !!v || 'الوصف مطلوب']" required outlined dense /> -->
                </v-col>
                <v-col cols="10" class="my-5">
                  <h6 class="text-center">Description</h6>
                  <vue-editor v-model="tier.description" placeholder="Description" :rules="[(v) => !!v || 'Description is required']" required outlined dense></vue-editor>
                  <!-- <v-textarea v-model="tier.description" placeholder="Description" :rules="[(v) => !!v || 'Description is required']" required outlined dense /> -->
                </v-col>
                <v-col cols="10" md="5">
                  <span>Precentage</span>
                  <v-text-field v-model="tier.percentage" hint="Percentage" type="number" :rules="[(v) => !!v || 'Percentage is required']" @input="setPercentage(index)" required outlined dense />
                </v-col>
                <v-col cols="10" md="5">
                  <span>Payout Net</span>
                  <v-text-field v-model="tier.payoutNet" hint="Payout Net" type="number" :rules="[(v) => !!v || 'Payout Net is required']" @input="setPayoutNet" required outlined dense/>
                </v-col>
                <v-col cols="10" md="5">
                  <span>Yalla Dealz Commission</span>
                  <v-text-field v-model="tier.yallaDealzCommission" hint="Yalla Dealz Commission" type="number" :rules="[(v) => !!v || 'Yalla Dealz Commission is required']" @input="setYDcommission" required outlined dense/>
                </v-col>
              </v-row>
            </v-col>
          </v-row>
      ...
      <v-row justify="center" class="text-center" dense>
        <v-col cols="10" md="5">
          ...
          <v-text-field v-model="dealData.voucher.validUntil" label="Valid Untill" type="datetime-local" @input="setVvaliduntill" outlined dense />
        </v-col>
        <v-col cols="10" md="5">
          <span>Minimum Quantity Allowed</span>
          <v-text-field v-model="dealData.voucher.minimumQuantityAllowed" label="Minimum Quantity Allowed" type="number" @input="setVmqa" outlined dense />
        </v-col>
        <v-col cols="10" md="5">
          <span>Maximum Quantity Allowed</span>
          <v-text-field v-model="dealData.voucher.maximumQuantityAllowed" label="Maximum Quantity Allowed" type="number" @input="setVmxqa" outlined dense />
        </v-col>
        <v-col cols="10" md="5">
          <span>User Allowed From</span>
          <v-text-field v-model="dealData.voucher.userAllowedFrom" label="User Allowed From" type="number" @input="setVuserallowedfrom" outlined dense />
        </v-col>
        <v-col cols="10" md="5">
          <span>User Allowed Upto</span>
          <v-text-field v-model="dealData.voucher.userAllowedUpto" label="User Allowed Upto" type="number" @input="setVuserallowedfupto" outlined dense />
        </v-col>
      </v-row>
      ...
      <v-row justify="center" class="text-center" dense>
        <v-col cols="10" md="5">
          ...
          <v-text-field v-model="dealData.voucher.minimumAge" label="Minimum Age" hint="Minimum Age" :rules="[(v) => !!v || 'Minimum Age is required']" required type="number" @input="setMinimumage" outlined dense />
        </v-col>
        ...
        <v-col cols="10" md="5">
          ...
          <v-text-field v-model="dealData.priorBooking.contactNumber" label="Contact Number" type="number" @input="setContactnumber" outlined dense />
        </v-col>
        <v-col cols="10" md="5">
          <span>Email</span>
          <v-text-field v-model="dealData.priorBooking.email" label="Email" @input="setEmail" outlined dense />
        </v-col>
        <v-col cols="10" md="5">
          <span>Delivery</span>
          <v-select v-model="dealData.priorBooking.delivery" label="Delivery" :items="Delivery" item-text="name" item-value="val" outlined dense />
        </v-col>
        <v-col cols="10" md="5">
          <span>Delivery Area</span>
          <v-text-field v-model="dealData.priorBooking.deliveryArea" label="Delivery Area" @input="setDeliveryarea" outlined dense />
        </v-col>
        <v-col cols="10" md="5">
          <span>Prior Booking Status</span>
          <v-select v-model="dealData.priorBooking.status" label="Prior Booking Status" :items="Prior" item-text="name" item-value="val" @change="setPrior" outlined dense />
        </v-col>
        <!-- 
        ...
        <v-col cols="10" v-if="!loading" class="d-flex justify-content-center">
          <v-btn color="primary" :preventClick="true" @click="confirmDeal" dark>Confirm Deal</v-btn>
        </v-col>
      </v-row>
    </v-form>
  </v-card>
    ...
    async confirmDeal() {
      ...
      try {
        if (!this.$refs.form.validate()) {
          ...
          if (this.isEdit) {
            if(this.dealData.branches.length && this.dealData.branches[0]._id) {
              const arrs = [];

              for(var i = 0; i < this.dealData.branches.length; i++) {
                arrs.push(this.dealData.branches[i]._id);
              }
              this.dealData.branches = arrs
            ...
            console.log('🚀 ~ file: dealForm.vue ~ line 559 ~ DealForm ~ confirmDeal ~ deepData', deepData)
