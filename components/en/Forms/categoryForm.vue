      ...
      <v-container class='mt-8'>
        <v-card class=" p-3" :loading="loading">
          ...
          <v-row justify="space-around" dense>
            <v-col cols="12" md="5">
              <v-text-field v-model="CategoryName" :rules="[(v) => !!v || 'Name is required']" label="Category Name" outlined dense required @input="setCategoryName"/>
            </v-col>

            <v-col cols="12" md="5">
              <v-text-field v-model="CategoryDesc" :rules="[(v) => !!v || 'Description is required']" label="Category Description" outlined dense  required @input="setCategoryDesc"/>
            </v-col>

            <v-col cols="12" md="5">
              <v-text-field v-model="ArCategoryName" :rules="[(v) => !!v || 'اسم القسم مطلوب']" label="اسم القسم" outlined dense  required @input="setArCategoryName"/>
            </v-col>

            <v-col cols="12" md="5">
              <v-text-field v-model="ArCategoryDesc" :rules="[(v) => !!v || 'وصف القسم مطلوب']" label="وصف القسم " outlined dense  required @input="setArCategoryDesc"/>
            </v-col>

            <v-col cols="12" md="11">
              <v-combobox v-model="Tags" :hint="Tags ? Tags + '' : 'No Tags Yet'" label="Add some tags" multiple persistent-hint small-chips outlined dense ></v-combobox>
            </v-col>

            <v-col class="d-flex" cols="12" md="11">
              <v-select v-model="ParentCategory" :items="items" label="Parent Category" outlined dense  @changedSelect="setParentCategory"></v-select>
            </v-col>

            <v-col cols="12" md="11">
              <div v-if="!image" class="en-CategoryForm-content-row-element">
                <v-btn color="success" class="" dark block>
              ...
              <v-row justify="center" class="text-center">
                <v-col cols="6" v-if="image">
                  ...
                  <v-btn color="red" class="" @click="removeImage" block dark>Remove Image</v-btn>
                </v-col>
              </v-row>
            </v-col>

            <v-col cols="11" class="d-flex justify-content-center mt-3">
              <v-btn color="primary" class="" dark @click="addCategeroy">Add Category</v-btn>
            </v-col>
          </v-row>
        </v-card>
      </v-container>
