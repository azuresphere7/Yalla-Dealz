  ...
  <v-container class="p-0">
    <v-row>
      <v-col class="ml-sm-2 mr-sm-6 mt-10 p-0">
        <v-card class="-xl p-3 text-center">
          <v-card-text>
            <v-form ref="mform" v-model="valid">
              <h4 class="d-inline">Merchant Data</h4>
              <v-row justify="center" class="mt-10" dense>
                <v-col cols="10" md="5">
                  <v-text-field v-model="merchantData.name" :success="!!merchantData.name" label="Merchant Name" :rules="[(v) => !!v || 'Merchant Name is required']" required outlined dense  />
                </v-col>
                 <v-col cols="10" md="5">
                  <v-text-field v-model="merchantData.arName" :success="!!merchantData.arName" label="Arabic Merchant Name" :rules="[(v) => !!v || 'Arabic Merchant Name is required']" required outlined dense  />
                </v-col>
                <v-col cols="10" md="5">
                  <v-text-field v-model="merchantData.phone" :success="!!merchantData.phone" label="Phone Number" :rules="[(v) => !!v || 'Phone Number is required',(v) =>(v && /^[0-9]+$/.test(v)) || 'Phone Number must be digits only']" required outlined dense  />
                </v-col>
                <v-col cols="10" md="5">
                  <v-text-field v-model="merchantData.email" :success="!!merchantData.email" label="Email" :rules="[(v) => !!v || 'E-mail is required', (v) => /.+@.+\..+/.test(v) || 'E-mail must be valid']" required outlined dense  />
                </v-col>
                <v-col cols="10" md="5">
                  <v-text-field v-model="merchantData.address" :success="!!merchantData.address" label="Address" :rules="[(v) => !!v || 'Address is required']" required outlined dense  />
                </v-col>
                <v-col cols="10" md="5">
                  <v-text-field v-model="merchantData.arAddress" :success="!!merchantData.arAddress" label="Address" :rules="[(v) => !!v || 'Arabic Address is required']" required outlined dense  />
                </v-col>
                <v-col cols="10" md="6">
                  <v-select v-model="merchantData.category" :success="!!merchantData.category" label="Category" :rules="[(v) => !!v || 'Category is required']" :items="flatCategories" item-value="_id" item-text="name" required outlined dense  block />
                </v-col>
              ...
              <v-row justify="center" class="mt-10" dense>
                <v-col cols="10" md="5">
                  <v-text-field v-model="merchantData.website" :success="!!merchantData.website" label="Website" outlined dense  />
                </v-col>
                <v-col cols="10" md="5">
                  <v-text-field v-model="merchantData.socialMediaLinks.facebook" :success="!!merchantData.socialMediaLinks.facebook" label="Facebook" outlined dense  />
                </v-col>
                <v-col cols="10" md="5">
                  <v-text-field v-model="merchantData.socialMediaLinks.youtube" :success="!!merchantData.socialMediaLinks.youtube" label="Youtube" outlined dense  />
                </v-col>
                <v-col cols="10" md="5">
                  <v-text-field v-model="merchantData.socialMediaLinks.instagram" :success="!!merchantData.socialMediaLinks.instagram" label="Instagram" outlined dense  />
                </v-col>
              </v-row>
              ...
              <v-row justify="center" class="mt-10" dense>
                <v-col cols="10" md="8">
                  <v-text-field v-model="merchantData.username" :rules="[(v) => !!v || 'Username is required']" label="Username" required outlined dense  />
                </v-col>
                <v-col cols="10" md="8">
                  <v-text-field v-model="merchantData.password" label="Password" outlined dense  />
                </v-col>
                <v-col cols="12"></v-col>
                <v-col cols="10" md="3" v-if="merchantData.image">
                  ...
                  <v-btn class="mt-3" color="red" btn-label="Remove Cover" @click="removeImage" dark dense >Remove Image</v-btn>
                </v-col>
                <v-col cols="10" md="8" v-else>
                  <v-btn color="success" class="-xl" dark block>
              ...
              <v-row justify="center" class="mt-5" dense>
                <v-row v-for="(payment,index) in merchantData.payments" :key="'p'+index" justify="center" class="mt-10" dense>
                  <v-col cols="10" md="5">
                    <v-text-field v-model="payment.amount" label="Amount" outlined dense  />
                  </v-col>
                  <v-col cols="10" md="5">
                    <v-text-field v-model="payment.dateGet" label="Date" type="datetime-local" outlined dense  />
                  </v-col>
                  <v-col cols="10" md="5">
                    <v-text-field v-model="payment.description" label="Description" outlined dense  />
                  </v-col>
                  <v-col cols="10" md="5">
                    <v-text-field v-model="payment.creatorWhom" label="By who" outlined dense  />
                  </v-col>
                  <v-col cols="10" md="5" class="d-flex justify-content-center">
                    <v-btn width="150" class="mr-2" color="red" @click="$delete(merchantData.payments, index)" dark dense >remove</v-btn>
                    <v-btn width="150" color="primary" @click="addPayment()" dense >Add Payment</v-btn>
                  </v-col>
                </v-row>
              </v-row>
            </v-form>
          ...
          <!-- <v-card-text>
            <v-btn width="150" color="primary" @click="merchantFormValidated" dense >Continue</v-btn>
          </v-card-text> -->
          ...
          <v-card-text>
            ...
            <v-form ref="pForm" v-model="valid">
              <v-row justify="center" class="mt-10" dense>
                <v-col cols="10" md="5">
                  <v-text-field v-model="merchantData.contactPerson.name" :counter="20" :success="!!merchantData.contactPerson.name" :rules="[(v) => !!v || 'Person Name is required', (v) => (v && v.length <= 20) || 'Name must be less than 20 characters']" label="Person Name" required outlined dense  />
                </v-col>
                <v-col cols="10" md="5">
                  <v-text-field v-model="merchantData.contactPerson.phone" :success="!!merchantData.contactPerson.phone" :rules="[(v) => !!v || 'Phone Number is required', (v) => (v && /^[0-9]+$/.test(v)) || 'Phone Number must be digits only']" label="Phone Number" required outlined dense  />
                </v-col>
                <v-col cols="10" md="5">
                  <v-text-field v-model="merchantData.contactPerson.email" :success="!!merchantData.contactPerson.email" label="Email" :rules="[(v) => !!v || 'E-mail is required', (v) => /.+@.+\..+/.test(v) || 'E-mail must be valid']" required outlined dense  />
                </v-col>
                <v-col cols="10" md="5">
                  <v-text-field v-model="merchantData.contactPerson.jobTitle" :success="!!merchantData.contactPerson.jobTitle" label="Job Title" :rules="[(v) => !!v || 'Job Title is required']" required outlined dense  />
                </v-col>
                <v-col cols="10">
                  <v-select v-model="merchantData.contactPerson.payOutBy" :success="!!merchantData.contactPerson.payOutBy" label="Payout By" :rules="[(v) => !!v || 'Payout by is required']" required :items="payOut" item-value="val" item-text="name" outlined dense  />
                </v-col>
                <v-col cols="5">
                  <v-btn block v-if="!branches.exist" :loading="loading" color="red" dark @click="contactPersonFormValidated" dense >Update</v-btn>
                </v-col>
...
export default {
  ...
  watch: {
    merchant() {
      ...
      this.merchantData.arName = this.merchant[0].arName;
      ...
      this.merchantData.arAddress = this.merchant[0].arAddress;
